...
class My_GUI():
    ...
    def re_print(self):
        ...
        for pcb_print in BackupReadyPCBs:
            pcb_list_Back_print.append(pcb_print.PName + '-->   ' + str(pcb_print.ReaminedTime) + '-->   ' + str(
    ...
    def print_log(self):
        ...
        for p in ReadyPCBs:
            ...
            ReadyPCB_string = ReadyPCB_string + p.PName + ' --> ' + str(p.ReaminedTime) + '\tnext:' + p.Plist[
                0].get_InstrucionId() + ':' + str(p.Plist[0].get_RunTime()) + '\t'
        print('BackupReadyPCBs:')
        for p in BackupReadyPCBs:
            print(p.PName, p.ReaminedTime, )
            BackupReadyPCBs_string = BackupReadyPCBs_string + p.PName + ' --> ' + str(p.ReaminedTime) + '\tnext:' + \
