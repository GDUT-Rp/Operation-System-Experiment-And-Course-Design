...
class Experiment:
    def __init__(self):
        ...
        self.root = Tk()
        self.root.title("操作系统实验_进程调度_3117004544_张润鹏")
        ...
        self.tree0.heading("name", text="进程名字")
        ...
        self.bottom_frame_fcfs_pin_quan_ = Label(self.frame_bottom, textvariable=self.var_fcfs_pin_quan_, font=('Arial', 12))
        ...
        self.bottom_frame_priority_pin_ = Label(self.frame_bottom_2, textvariable=self.var_priority_pin_, font=('Arial', 12))
    ...
    def run_rr(self):
        self.process_ready_list.sort(key=functools.cmp_to_key(lambda x, y: x.commit < y.commit))
        tmp_ = self.process_ready_list.copy()
        self.q = int(self.var_shijianpian.get())
        q = self.q
        self.get_tree_rr()
        while len(self.process_ready_list.copy()) > 0:
            tmp_ = self.process_ready_list.copy()
            for process in tmp_:
                self.var_id = process.id
                self.var_name = process.name
                ...
                self.var_source = process.source
                self.var_state = 'Run'
                times = int(process.need)
                tmp = process.need
                if process.end == 0:
                    process.begin = self.time
                else:
                    times -= q
                    tmp = process.need - tmp
                for i in range(times):
                    ...
                    sleep(1)
                process.end = self.time
                print('process.end:', self.time)
                if int(process.need) <= 3 or process.flag:
                    process.circle = process.end - process.commit
                    process.quan = process.circle / process.service
                    process.state = 'Finish'
                    self.process_finish_list.append(process)
        ...
        self.get_tree_rr()
        self.ing_init()
        print('len:', len(self.process_finish_list))
    ...
    def button_get_value(self):
        ...
        for elem in self.process_ready_list:
            if elem.name == process.name:
                tkinter.messagebox.showinfo('提示', '添加失败，当前进程已在就绪队列')
                flag = False
        if flag:
